...
const ChatContainer = () => {
  ...
  const isSelectedUserOnline = selectedUser ? (onlineUser || []).includes(selectedUser._id) : false;
  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      ...
      <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-500'>
        ...
        <p className='flex-1 text-lg text-white flex items-center gap-2'>{selectedUser.fullName}
          <span className={`w-2 h-2 rounded-full ${isSelectedUserOnline ? 'bg-green-500' : 'bg-neutral-500'}`}></span>
        </p>
    ...
    <div onClick={handleSendMessage} className='flex flex-col items-center justify-center gap-2 text-gray-500 bg-white/10 max-md:hidden'>
